...
module private TypesFactory =
    ...
    let private generateChangedEvent =
       ...
       fun() ->
           ...
           let event = ProvidedEvent("Changed", 
                                     eventType,
    ...
    and transformList readOnly name generateField (children: Node list) =
       ...
       if generateField then
            ...
            let prop =
                if not readOnly
                then ProvidedProperty (name, propType, isStatic=false, getterCode = (fun [me] -> Expr.Coerce(Expr.FieldGet(me, field), propType)), setterCode = (fun [me; v] -> Expr.FieldSet(me, field, Expr.Coerce(Expr.Call(listCtr, [Expr.Coerce(v, ctrType)]), fieldType))))
...
let internal typedYamlConfig (context: Context) assy =
   let baseTy = typeof<Root>

   let yamlConfig = ProvidedTypeDefinition(assy, rootNamespace, "YamlConfig", Some baseTy, isErased=false, SuppressRelocation=false)
       ...
       instantiationFunction = fun typeName paramValues ->
           let value = lazy (
               let createTy yaml readOnly inferTypesFromStrings =
                   let ty = ProvidedTypeDefinition (assy, rootNamespace, typeName, Some baseTy, isErased=false,
